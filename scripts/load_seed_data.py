...
from transportations_validator.models.parameter import Parameter, ParameterAlias, FacilityType, DataType
        ...
        for param_data in data["parameters"]:
            ...
            result = await session.execute(
                select(Parameter).where(
                    Parameter.rust_field == rust_field,
                    Parameter.facility_type == facility_type
                )
            )
            existing = result.scalar_one_or_none()
...
async def load_rules(
    ...
    """Load design rules from seed files."""
    print("Loading rules...")

    rules_file = SEED_DATA_DIR / "rules" / "hcm_rules.json"
    ...
    for rule_data in data["rules"]:
        ...
        if key not in param_ids:
            print(f"  Warning: Parameter {key} not found, skipping rule")
            continue
        ...
        result = await session.execute(
            select(DesignRule).where(
                DesignRule.parameter_id == param_id,
                DesignRule.name == rule_data["name"]
            )
        )
...
async def main() -> None:
    ...
    async with async_session_maker() as session:
        condition_ids = await load_conditions(session)
